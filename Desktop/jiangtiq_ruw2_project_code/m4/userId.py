        ...
        try:
            ...
            if d.has_key('items'):            
                for comment in d['items']:
                    ...
                    write_dict['comment_id'] = comment['id']
                    write_dict['victory'] = comment['petition']['is_victory']
                    ...
                    write_dict['description'] = comment['petition']['description']
                    write_dict['creator'] = comment['petition']['user']['id']
                    write_dict['tags'] = comment['petition']['tags']
    
                    open('r_animal-welfare-6.txt','a').write("%s\n" % write_dict)
                if (d["last_page"] == True):
                    #print "done"
                    break
                else:
